...
URL_t *urlOpen(char *fname, CURLcode (*callBack)(CURL*), const char *mode) {
    ...
    if((!mode) || (strchr(mode, 'w') == 0)) {
        ...
        if(URL->type == BWG_FILE) {
            ...
            if(curl_easy_setopt(URL->x.curl, CURLOPT_PROXY_SSL_VERIFYPEER, 0) != CURLE_OK) {
                fprintf(stderr, "[urlOpen] Couldn't set CURLOPT_PROXY_SSL_VERIFYPEER to 0!\n");
                goto error;
            }
            if(curl_easy_setopt(URL->x.curl, CURLOPT_PROXY_SSL_VERIFYHOST, 0) != CURLE_OK) {
                fprintf(stderr, "[urlOpen] Couldn't set CURLOPT_PROXY_SSL_VERIFYHOST to 0!\n");
                goto error;
            }
            if(callBack) {
